...
namespace xpx_chain_sdk::internal::network {
    ...
    WsClient::WsClient(
            ...
            _outgoingQueue(),
            _io_context(context),
            _resolver(boost::asio::make_strand(*_io_context)),
            ...
            auto disconnectTask = [pThis]() {
                if (pThis->isConnected()) {
                    pThis->_ws.async_close(boost::beast::websocket::close_code::normal,
                                           boost::asio::bind_executor(*pThis->_io_context, [pThis](auto errorCode) {
    ...
    void WsClient::send(const std::string& json, std::function<void()> onSuccess,
        ...
        boost::asio::post(*_io_context, [pThis = shared_from_this(), json, onSuccess, onError] {
    ...
    void WsClient::doWrite() {
        _ws.async_write(
                ...
                boost::asio::bind_executor(*_io_context, [pThis = shared_from_this()](auto errorCode, auto bytesTransferred) {
                    if (errorCode) {
                        boost::asio::post(*pThis->_io_context, [errorCode, onError = pThis->_outgoingQueue[0].second.second]() {
                            if (onError) {
                                onError(errorCode);
                            }
                        });
                    ...
                    boost::asio::post(*pThis->_io_context, [onSuccess = pThis->_outgoingQueue[0].second.first]() {
                        if (onSuccess) {
                            onSuccess();
                        }
                    });
